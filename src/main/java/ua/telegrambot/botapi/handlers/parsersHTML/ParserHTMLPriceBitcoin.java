...
@Component
public class ParserHTMLPriceBitcoin implements ParserHTMLPrice {

	private Document document;
	private Pattern pattern = Pattern.compile("\\d{2}\\,\\d{3}\\.\\d{2}");
	...
	public Document getWebPage(){
		try {
			document = Jsoup.connect("https://www.coindesk.com/price/bitcoin").get();
		} catch (IOException e){
			e.getStackTrace();
		}
		return document;
	...
	public String getPrice(){
		...
		String price = "Bitcoin: Дата " + dateFormat.format(new Date()) + ".Цена " + Emojis.CHART_UP + "$" + getPriceFromString(priceString)
				+ Emojis.CHART_DOWN;
