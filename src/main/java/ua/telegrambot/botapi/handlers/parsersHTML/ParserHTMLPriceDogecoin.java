...
@Component
public class ParserHTMLPriceDogecoin implements ParserHTMLPrice {

	private Document document;
	private Pattern pattern = Pattern.compile("\\d\\.\\d{6}");
	...
	public Document getWebPage(){
		try {
			document = Jsoup.connect("https://www.coindesk.com/price/dogecoin").get();
		} catch (IOException e){
			e.getStackTrace();
		}
		return document;
	...
	public String getPrice(){
		...
		String price = "Dogecoin: Дата " + dateFormat.format(new Date()) + ".Цена " + Emojis.CHART_UP + "$" + getPriceFromString(priceString) +
				Emojis.CHART_DOWN;
